...
void ChatSharing::sendChatMessage(const String &text) {
    ...
    // dstAddress is either broadcastAddress or a specific peer's MAC,
    // whichever the user picked in the peer menu at the start of run().
...
void ChatSharing::run() {
    ...
    // plus any device that replies to the ping - exactly the same picker
    // FileSharing uses before sending a file. Whatever the user selects ends
    // up in dstAddress (already registered as an ESP-NOW peer). Its
    // destructor (~EspConnection) tears the radio down again automatically
    // when this object goes out of scope, so cleanup is guaranteed even on
    // early return/ESC.
    if (!beginSend()) return;

    bool isBroadcast = memcmp(dstAddress, broadcastAddress, 6) == 0;
    chatTitle         = isBroadcast ? "Chat: Broadcast" : "Chat: " + macToString(dstAddress);
