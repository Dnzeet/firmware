...
// FIX(Bug#3): Store active NRF24 mode so stopBLEJammer() doesn't call
// nrf_setMode() (which shows an interactive menu) during cleanup.
...
bool isNRF24Available() {
    if (!nrf24Initialized) {
        ...
        if (nrf_start(mode)) {
            ...
            activeNRFMode = mode; // FIX(Bug#3): Save mode for use in stopBLEJammer()
...
void stopBLEJammer() {
    if (!bleJammingActive) return;
    // FIX(Bug#3): Use saved activeNRFMode instead of calling nrf_setMode()
    // which would show an interactive menu popup during cleanup — wrong UX
    // and potentially uses a different mode than what was used to start.
    if (CHECK_NRF_SPI(activeNRFMode)) NRFradio.stopConstCarrier();
