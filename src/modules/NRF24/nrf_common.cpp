...
RF24 NRFradio(NRF24_CE_PIN, NRF24_SS_PIN); // Pins passed via begin() — avoids static init order fiasco
...
bool nrf_start(NRF24_MODE mode) {
    ...
    // CE pre-init: set LOW before begin() for SPI bus stability (same as old firmware).
    // begin() receives the actual ce_pin via rf24_gpio_pin_t(bruceConfigPins.NRF24_bus.io0)
    // so the library will correctly manage CE HIGH/LOW after begin() completes.
    // startConstCarrier() / startListening() will override this LOW state via library ce() calls.
