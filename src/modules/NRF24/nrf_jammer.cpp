...
static const uint8_t CH_ZIGBEE[] = {
    ...
    69, 70, 71, // ch24
    74, 75, 76, // ch25
    79, 80, 81  // ch26 — note: nRF24 ch81=2481MHz, within ISM band
};
...
static void floodChannel(uint8_t ch, uint16_t dwellMs) {
    // Let RF24 library manage CE pin — manual digitalWrite conflicts
    // with library's internal CE state tracking
    NRFradio.stopListening();
    NRFradio.flush_tx();
    NRFradio.setChannel(ch);
    delayMicroseconds(200); // Wait for PLL re-lock (~130us per datasheet)
    ...
    if (dwellMs == 0) {
        ...
        NRFradio.writeFast(JAM_FLOOD_DATA, 32, true);
        NRFradio.writeFast(JAM_FLOOD_DATA, 32, true);
        NRFradio.txStandBy(); // Flush TX FIFO, prevent overflow on next burst
        return;
    }
    ...
    while ((millis() - startMs) < dwellMs) {
        if (!NRFradio.writeFast(JAM_FLOOD_DATA, 32, true)) {
            NRFradio.txStandBy(); // Clear stalled FIFO before retrying
            delayMicroseconds(10);
        }
    }
    NRFradio.txStandBy(); // Always flush at end of dwell
}
...
static void initCW(int channel) {
    // Old code comment: "we start at 50 as on lower channel the cw wont work correctly"
    // Always init CW at ch50 or higher to ensure carrier works reliably
    int initCh = (channel < 50) ? 50 : channel;
    NRFradio.powerUp();
    delay(15); // Tpd2stby: nRF24 datasheet worst-case 10ms + margin
    NRFradio.setPALevel(RF24_PA_MAX);
    NRFradio.startConstCarrier(RF24_PA_MAX, initCh);
...
// ── CW on a channel ─────────────────────────────────────────────
// Kode lama proven: startConstCarrier sekali saat init, lalu
// cukup setChannel() untuk hop. PLL re-lock ~130µs, carrier
// tidak pernah mati → duty cycle maksimal.
static void cwChannel(uint8_t ch, uint16_t dwellMs) {
    NRFradio.setChannel(ch);
    // Tidak perlu re-init atau stopConstCarrier — sama persis kode lama
...
static void runJammer(NRF24_MODE nrfMode, NrfJamMode jamMode) {
    ...
    // FIX(Bug#4): hopIndex and channel must be in sync with CW init.
    // Previously channel was clamped to >=50 for CW stability but hopIndex
    // stayed at 0 (pointing to initChannels[0] which may be <50), causing
    // the first jammer iteration to hop to a different channel than CW was
    // initialized on — unstable carrier on first sweep.
    //
    // Fix: find the first channel in the list that is >=50, set hopIndex
    // to that index so the loop starts exactly where CW was initialized.
    size_t initCount;
    const uint8_t *initChannels = getChannelList(currentMode, initCount);

    // Find starting hopIndex: first channel >= 50, or 0 if none qualify
    int startHopIndex = 0;
    if (initCount > 0 && initChannels) {
        for (size_t idx = 0; idx < initCount; idx++) {
            if (initChannels[idx] >= 50) {
                startHopIndex = (int)idx;
                break;
            }
        }
    }
    hopIndex = startHopIndex;

    int channel = (initCount > 0 && initChannels) ? initChannels[startHopIndex] : 50;
    if (channel < 50) channel = 50; // Fallback safety for FULL/DRONE modes

    if (CHECK_NRF_SPI(nrfMode)) {
        NrfJamConfig &cfg = jamConfigs[(uint8_t)currentMode];
        bool flooding = cfg.useFlooding;

        // NOTE: For CW mode, initCW() was already called in nrf_jammer() immediately
        // after nrf_start() to minimize CE LOW→HIGH delay. We only need to re-apply
        // config here for flooding mode (which has different setup than CW).
        if (flooding) {
            applyJamConfig(cfg, true);
        }
        // CW: already active from pre-runJammer initCW() call, no action needed.
        NRFSPI = 1;
    }
    ...
    while (running) {
        ...
        if (check(NextPress)) {
            ...
            if (CHECK_NRF_SPI(nrfMode)) {
                ...
                NrfJamConfig &cfg = jamConfigs[(uint8_t)currentMode];
                // Ikuti pola kode lama: kalau sama-sama CW, cukup lanjut setChannel()
                // Stop/re-init hanya kalau strategy berubah (CW↔Flood)
...
void nrf_jammer() {
    ...
    // FIX(Bug#5-REAL): Call initCW() IMMEDIATELY after nrf_start() before any
    // other logic runs. nrf_start() leaves CE=LOW (hardware). The longer the gap
    // between nrf_start() and startConstCarrier(), the more likely CE desync causes
    // no RF output. Old firmware did: nrf_start() → startConstCarrier() with zero
    // code in between. Mirror that behavior here for CW mode.
    if (CHECK_NRF_SPI(nrfMode)) {
        NrfJamConfig &cfg = jamConfigs[(uint8_t)selectedMode];
        if (!cfg.useFlooding) {
            // Get first suitable channel for this mode (>=50 for CW stability)
            size_t preCount;
            const uint8_t *preCh = getChannelList(selectedMode, preCount);
            int preCwCh = 50;
            if (preCount > 0 && preCh) {
                for (size_t idx = 0; idx < preCount; idx++) {
                    if (preCh[idx] >= 50) { preCwCh = preCh[idx]; break; }
                }
            }
            initCW(preCwCh); // CE goes HIGH immediately after nrf_start
        }
    }
