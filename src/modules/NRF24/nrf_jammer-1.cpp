...
static void runJammer(NRF24_MODE nrfMode, NrfJamMode jamMode) {
    ...
    // Set initial channel based on mode (avoid showing CH:0 on first draw)
    // Start at ch50 minimum — old code: "lower channel the cw wont work correctly"
    size_t initCount;
    const uint8_t *initChannels = getChannelList(currentMode, initCount);
    int channel = (initCount > 0 && initChannels) ? initChannels[0] : 50;
