...
String scanChannels(bool web) {
    ...
    // CE toggle: set LOW before sweep, let startListening/stopListening handle per-channel.
    // After nrf_start() + begin(), library owns the CE pin — manual toggle here is safe
    // because begin() already registered the pin. This matches old firmware behavior
    // (old used NRF24_CE_PIN directly; we use the equivalent bruceConfigPins.NRF24_bus.io0).
...
void nrf_spectrum() {
    ...
    // FIX(Bug#1): Added nrf_setMode() so user can select SPI/UART/BOTH
    // before init. Previously hardcoded NRF_MODE_SPI caused silent init
    // failure on boards where SPI bus needs proper caller-side selection
    // (e.g. shared bus, UART bridge). Mirrors pattern used in nrf_jammer().
