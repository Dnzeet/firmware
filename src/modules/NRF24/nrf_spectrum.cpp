...
String scanChannels(bool web) {
    ...
    // FIX(Bug#2): Removed manual digitalWrite(CE, LOW/HIGH).
    // Manual CE toggling conflicts with RF24 library's internal CE state
    // tracking, causing startListening()/stopListening() to malfunction
    // and RPD to always return 0 (spectrum blank). Library manages CE internally.
