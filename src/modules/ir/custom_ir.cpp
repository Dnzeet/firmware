#include "custom_ir.h"
#include "TV-B-Gone.h" // for checkIrTxPin(), StartTvBGoneForRegion()
#include "ir_read.h"   // for IrRead::captureOneSignal()
...
// Same file-parsing + picker UI as chooseCmdIrFile(), but hands the chosen
// IRCode back to the caller via outCode instead of transmitting it right
// away. Used by IR Timed Transmit, which needs to hold the code until a
// delay countdown finishes before firing it.
int pickCmdIrFile(FS *fs, const String &filepath, IRCode &outCode) {
    ...
    if (!databaseFile) {
        Serial.println("Failed to open IR file.");
        return 1; // treat like "back to file browser"
    }
    Serial.println("Opened IR file.");

    // Parse the file into one IRCode per "name:" block -- identical
    // parsing rules to chooseCmdIrFile() so both stay in sync.
    String line;
    String txt;
    codes.push_back(new IRCode());
    ...
    bool actionTaken = false;
    bool picked = false;
    IRCode *selected = nullptr;

    for (auto code : codes) {
        if (code->name != "") {
            options.push_back({code->name.c_str(), [code, &actionTaken, &picked, &selected, &exit]() {
                                   actionTaken = true;
                                   picked = true;
                                   selected = code;
                                   exit = true;
    ...
    if (picked && selected != nullptr) { outCode = IRCode(selected); }
    ...
    if (!goToMainMenu) { returnToMenu = false; }

    if (picked) return 0;
    return goToMainMenu ? 2 : 1;
}

// ============================================================
//  IR Timed Transmit
//  Pick a source (Custom IR file / TV-B-Gone / Record New Signal),
//  pick a delay, then wait -- the pick fires automatically once the
//  countdown reaches zero. ESC during the countdown cancels cleanly.
// ============================================================

namespace {

enum class TimedIrSource { NONE, CUSTOM_FILE_ALL, CUSTOM_FILE_CMD, TVBGONE, RECORDED };

// Shows a delay picker (presets + custom) and returns seconds, or -1 if
// the user backed out.
int pickDelaySeconds() {
    int minutes = -1;
    options = {
        {"5 minutes",  [&]() { minutes = 5; }  },
    ...
    if (minutes == 0) {
        String txt = num_keyboard("10", 3, "Minutes (1-999):");
        if (txt == "\x1B" || txt == "") return -1;
        minutes = txt.toInt();
...
// Blocking countdown with a live progress bar. ESC cancels (returns
// false). Returns true once the full duration has elapsed.
// NOTE: deliberately does NOT use the shared progressHandler() helper --
// at low progress values it clears a large chunk of the screen and draws
// its own status banner, which would fight with the countdown number
// drawn above it. This bar is self-contained instead.
bool runCountdown(int totalSeconds, const String &label) {
    unsigned long startMs = millis();
    unsigned long endMs = startMs + (unsigned long)totalSeconds * 1000UL;
    int lastShown = -1;
    ...
    while (true) {
        ...
        long leftMs = (long)(endMs - millis());
        if (leftMs <= 0) break;

        int leftSec = (leftMs + 999) / 1000;
...
// ---- branch: Custom IR file --------------------------------------
// Returns true if a source was picked (fs/filepath/code/spamAll filled),
// false if the user backed out entirely.
bool pickCustomIrSource(TimedIrSource &source, FS *&fs, String &filepath, IRCode &code) {
    checkIrTxPin();
    fs = NULL;
    ...
    while (true) {
        ...
        options = {
            {"Choose cmd", [&]() { spamAll = false; }},
            {"Spam all",   [&]() { spamAll = true; } },
            {"Menu",       [&]() { exitPicked = true; }},
        };
        loopOptions(options);
        if (exitPicked) return false;

        if (spamAll) {
            source = TimedIrSource::CUSTOM_FILE_ALL;
            return true;
        }

        int result = pickCmdIrFile(fs, filepath, code);
        if (result == 0) {
            source = TimedIrSource::CUSTOM_FILE_CMD;
            return true;
        }
        if (result == 2) return false; // user backed all the way out
        // result == 1: back to file browser, loop again
    }
}

// ---- branch: TV-B-Gone ---------------------------------------------
bool pickTvBGoneRegion(uint8_t &region) {
    bool picked = false;
    options = {
        {"Region NA", [&]() { region = 1; picked = true; }}, // matches NA in TV-B-Gone.cpp
        {"Region EU", [&]() { region = 0; picked = true; }}, // matches EU in TV-B-Gone.cpp
        {"Menu",      yield                                },
    };
    loopOptions(options);
    return picked;
...
// ---- branch: Record New Signal --------------------------------------
bool recordNewSignal(IRCode &code) {
    IrRead irReader(true /* headless: skip its own source-type menu */);
    return irReader.captureOneSignal(code);
}

} // namespace

void timedIrTransmitMenu() {
    TimedIrSource source = TimedIrSource::NONE;
    FS *fs = NULL;
    String filepath = "";
    IRCode code;
    uint8_t region = 1;
    ...
    bool ready = false;
    options = {
        {"Custom IR File",     [&]() { ready = pickCustomIrSource(source, fs, filepath, code); }},
        {"TV-B-Gone",
         [&]() {
             ready = pickTvBGoneRegion(region);
             if (ready) source = TimedIrSource::TVBGONE;
         }                                                                                       },
        {"Record New Signal",
         [&]() {
             ready = recordNewSignal(code);
             if (ready) source = TimedIrSource::RECORDED;
         }                                                                                       },
    };
    addOptionToMainMenu(); // adds "Main Menu" -- same convention as IRMenu::optionsMenu()
    loopOptions(options);

    if (!ready || source == TimedIrSource::NONE) return;

    int totalSeconds = pickDelaySeconds();
    ...
    String label;
    switch (source) {
        case TimedIrSource::CUSTOM_FILE_ALL: label = "All codes: " + filepath; break;
        case TimedIrSource::CUSTOM_FILE_CMD: label = "Cmd: " + code.name; break;
        case TimedIrSource::TVBGONE:         label = region == 1 ? "TV-B-Gone (NA)" : "TV-B-Gone (EU)"; break;
        case TimedIrSource::RECORDED:        label = "Recorded signal"; break;
        default:                             label = ""; break;
    }
    tft.println(label);
    tft.println("Delay: " + String(totalSeconds / 60) + " min");
    tft.println("");
    tft.println("Will fire automatically.");
    tft.println("SEL=start  ESC=cancel");
    ...
    bool completed = runCountdown(totalSeconds, label);

    drawMainBorder();
    if (!completed) {
        displayRedStripe("Cancelled");
        delay(1200);
        return;
    }

    tft.setTextSize(FM);
    tft.drawCentreString("Firing...", tftWidth / 2, 50, 1);

    switch (source) {
        case TimedIrSource::CUSTOM_FILE_ALL: txIrFile(fs, filepath); break;
        case TimedIrSource::CUSTOM_FILE_CMD:
        case TimedIrSource::RECORDED:        sendIRCommand(&code, true); break;
        case TimedIrSource::TVBGONE:         StartTvBGoneForRegion(region); break;
        default: break;
    }

    drawMainBorder();
    tft.setTextSize(FM);
    tft.drawCentreString("Done!", tftWidth / 2, 50, 1);
    delay(1200);
}
